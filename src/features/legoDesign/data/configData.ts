...
// BIAN dataset provided by user
export const bianData: any[] = [
	{
	...
	},
	{
		...
		businessDomain: [
			...
			{
				uid: "BIAN-13.0.0-ChannelManagement",
				name: "Channel Management",
				serviceDomain: [
					{
						uid: "BIAN-13.0.0-AdvancedVoiceServicesManagement",
						name: "Advanced Voice Services Management",
					},
					{
						uid: "BIAN-13.0.0-ATMNetworkManagement",
						name: "ATM Network Management",
					},
					{
						uid: "BIAN-13.0.0-BranchCurrencyManagement",
						name: "Branch Currency Management",
					},
					{
						uid: "BIAN-13.0.0-BranchLocationManagement",
						name: "Branch Location Management",
					},
					{
						uid: "BIAN-13.0.0-BranchNetworkManagement",
						name: "Branch Network Management",
					},
					{
						uid: "BIAN-13.0.0-BranchPortfolio",
						name: "Branch Portfolio",
					},
					{
						uid: "BIAN-13.0.0-CentralCashHandling",
						name: "Central Cash Handling",
					},
					{
						uid: "BIAN-13.0.0-ChannelActivityAnalysis",
						name: "Channel Activity Analysis",
					},
					{
						uid: "BIAN-13.0.0-ChannelPortfolio",
						name: "Channel Portfolio",
					},
					{
						uid: "BIAN-13.0.0-ContactCenterManagement",
						name: "Contact Center Management",
					},
					{
						uid: "BIAN-13.0.0-eBranchManagement",
						name: "eBranch Management",
					},
					{
						uid: "BIAN-13.0.0-FinancialMessageAnalysis",
						name: "Financial Message Analysis",
					},
					{
						uid: "BIAN-13.0.0-InformationProviderAdministration",
						name: "Information Provider Administration",
					},
					{
						uid: "BIAN-13.0.0-MarketDataSwitchAdministration",
						name: "Market Data Switch Administration",
					},
					{
						uid: "BIAN-13.0.0-ProductInventoryItemManagement",
						name: "Product Inventory Item Management",
					},
					{
						uid: "BIAN-13.0.0-ServicingActivityAnalysis",
...
// Transform the BIAN hierarchy into the content shape expected by the Lego layout
export function mapBianToContentData(data: any[]): Record<string, any> {
	...
	data.slice(0, 8).forEach((topLevel, index) => {
		const itemId = `item${index + 1}`;
		content[itemId] = {
			label: topLevel.name,
			backgroundColor: backgroundPalette[index % backgroundPalette.length],
			childLayout: "one-column",
			children: (topLevel.businessDomain || []).map(
				(bd: any, bdIdx: number) => ({
					subItem: bd.name,
					grids: (bd.serviceDomain || []).map((sd: any, sdIdx: number) => ({
						id: sdIdx + 1,
						uid: sd.uid,
						label: sd.name,
						// Use a simple 1x1 block so each service domain is a single tile when zoomed in
						blocks: [{ rows: 1, cols: 1 }],
					})),
				})
			),
		};
